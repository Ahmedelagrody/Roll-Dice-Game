...
const calcRoll = function (arr_current) {
  let random = Math.trunc(Math.random() * 6) + 1

  diceEl.src = `dice-${random}.png`

  let added_Random = 0
  arr_current.push(random)

  if (random !== 1) {
    for (let i = 0; i <= arr_current.length - 1; i++) {
      added_Random += arr_current[i]
    }
    return added_Random
  } else {
    arr_current.splice(0, arr_current.length, 0)

    if (arr_current === arr_current1) {
      player_one.classList.remove('player--active')
      player_two.classList.add('player--active')
      return (added_Random = 0)
    } else if (arr_current === arr_current2) {
...
const rollDice_btn = function () {
  if (
    ...
    if (player_one.classList.contains('player--active')) {
      added_Random1 = calcRoll(arr_current1)
      current_score_player1.textContent = added_Random1
    } else {
      added_Random2 = calcRoll(arr_current2)
...
const New_Game = function () {
  ...
  playerName_2.textContent = 'Player 2'
